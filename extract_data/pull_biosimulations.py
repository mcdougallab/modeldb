...
data = requests.get("https://raw.githubusercontent.com/biosimulations/biosimulations-modeldb/dev/biosimulations_modeldb/final/status.yml").text
models = re.findall("modeldb-(\d+)", data)

for model in models:
    sdb.models.update_one({"id": int(model)}, {"$set": {"biosimulations": {"value": f"modeldb-{model}"}}})
